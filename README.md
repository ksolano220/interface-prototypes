...
**`wayfare-hud.html`** — A working walk-planning app: set a step goal, pick a time window, tap glowing points on an illustrated SoHo/Hudson River map to compose a route live. Circular gauges track distance, steps, and time against the goal. Styled as a holographic HUD per a specific visual reference, rather than a generic take.
...
- `wayfare-hud.html` and `no-asterisks.html` are designed mobile-first (open on a phone, or view the desktop preview inside the phone-frame mockup).
